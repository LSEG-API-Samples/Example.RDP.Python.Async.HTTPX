...
async def get_historical_interday_summaries(ric, token, url, client, interval, start, end, fields):
    """Request historical Interday summaries data"""
    ...
    # parameters for interday-summaries request.
    params = {
        "interval": interval,
        "start": start,
        "end": end,
    ...
    #querystring = {"interval":"P1D","start":"2019-01-01","end":"2019-01-31","fields":"TRDPRC_1,BID,ASK","adjustments":"exchangeCorrection,manualCorrection,CCH,CRE,RPO,RTS"}
    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            for ric in HISTORICAL_RICS:
                history_data = await get_historical_interday_summaries(ric, access_token, HISTORICAL_INTERDAY_SUMMARIES_URL, client, interval="P1D", start=start, end=end, fields=fields)
                print("Historical interday summaries retrieved successfully!")
                print(f"Historical interday summaries for {ric}:\n{json.dumps(history_data, indent=2)}\n")
